...
class CustomNormalize(object):
    ...
    def __call__(self, tensor):
        if tensor.dim() == 3:  # [C,H,W]
            mean = torch.tensor(self.mean, device=tensor.device, dtype=tensor.dtype)[:, None, None]
            std  = torch.tensor(self.std,  device=tensor.device, dtype=tensor.dtype)[:, None, None]
